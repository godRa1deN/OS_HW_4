...
int main(int argc, char *argv[]) {
    struct sockaddr_in server_address, client_address, state_address;
    socklen_t client_address_len, state_address_len;
    ...
    printf("Сервер запущен и ожидает подключение state программы...\n");

    memset(buffer, 0, MAX_BUFFER_SIZE);
    state_address_len = sizeof(state_address);
    recv_len = recvfrom(server_socket, buffer, MAX_BUFFER_SIZE, 0,
                        (struct sockaddr*)&state_address, &state_address_len);
    if (recv_len == -1) {
        perror("Ошибка приема данных");
        close(server_socket);
        exit(1);
    ...
    while (connected_clients < num_clients) {
        ...
        snprintf(state_message, sizeof(state_message), "Сервер: Подключено клиентов: %d", connected_clients + 1);
        if (sendto(server_socket, state_message, strlen(state_message), 0,
                   (struct sockaddr*)&state_address, state_address_len) == -1) {
            perror("Ошибка отправки состояния сервера");
            close(server_socket);
            exit(1);
        }
        ...
        //Прием сообщения клиента state программе
        memset(buffer, 0, MAX_BUFFER_SIZE);
        recv_len = recvfrom(server_socket, buffer, MAX_BUFFER_SIZE, 0,
                    (struct sockaddr*)&client_address, &client_address_len);
        if (recv_len == -1) {
        ...
        //Передача сообщения state программе
        ...
        //Передача сообщения state программе
        char *client_encoded_message = "Сервер получил закодированный массив\n";
        if (sendto(server_socket, client_encoded_message, strlen(client_encoded_message), 0,
                   (struct sockaddr*)&state_address, state_address_len) == -1) {
            perror("Ошибка отправки состояния сервера");
            close(server_socket);
            exit(1);
    ...
    char *exit_message = "SERVER_EXIT";
    if (sendto(server_socket, exit_message, strlen(exit_message), 0,
                (struct sockaddr*)&state_address, state_address_len) == -1) {
        perror("Ошибка отправки состояния сервера");
        close(server_socket);
        exit(1);
